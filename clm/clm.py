...
def main():
    ...
    parser.add_argument('--min_artist_cnt', type=int, default=20,
    ...
    for epoch in range(args.epochs):
        for batch_idx, (batch_X, batch_Y) in enumerate(clm_data.get_transformed_batches(vectorizers, endless=False)):
            ...
            if batch_idx % args.log_interval == 0 and batch_idx > 0:
                ...
                rnd_conds = {}
                for c in conditions:
                    rnd_conds[c] = torch.LongTensor([[vectorizers['artists'].syll2idx['que']]]).to(device)
